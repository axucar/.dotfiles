...
# Function to get git branch and rename tmux window
set_tmux_window_name() {
  ...
  if [[ -n "$TMUX" ]]; then
    ...
    if [[ -n "$branch" ]]; then
      # If in a git repo, set window name to the branch
      tmux rename-window "$branch"
    else
      # If not in a git repo, set it to the current directory name
